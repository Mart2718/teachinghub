const HUBS = [
  {
    id: "stat-c1000",
    course: "STAT C1000",
    courseLabel: "Introductory Statistics",
    color: "#0e7490",
    icon: "📊",
    sites: [
      {
        label: "Statistical Learning Tools Hub",
        url: "https://sacstatc1000learninghub.netlify.app/",
        description: "Central hub of interactive web apps for STAT C1000 — simulations, inference tools, and data explorations.",
        tags: ["apps", "inference", "simulation"]
      }
    ]
  },
  {
    id: "math-204",
    course: "Math 204",
    courseLabel: "Mathematics for Elementary Teachers",
    color: "#7c3aed",
    icon: "✏️",
    sites: [
      {
        label: "Inteactive Apps for Learning",
        url: "https://mfetwebapps.netlify.app/",
        description: "Interactive web apps designed for Math 204 — number sense, base systems, geometry, and elementary math concepts.",
        tags: ["apps", "elementary", "number sense"]
      }
    ]
  },
  {
    id: "teaching",
    course: "Teaching and Learning",
    courseLabel: "Apps for Teaching",
    color: "#b45309",
    icon: "📐",
    sites: [
      {
        label: "Intructional Material",
        url: "https://tinyteacherapps.netlify.app/ ",
        description: "Interactive web apps designed for classroom use.",
        tags: ["apps", "teaching", "learning", "instruction"]
      }
    ]
  },
  {
    id: "calc-bridge",
    course: "CalcBridge Apps",
    courseLabel: "Algebra, Trig & Calculus Prep",
    color: "#065f46",
    icon: "∫",
    comingSoon: true,
    sites: [
      {
        label: "CalcBridge Math Apps",
        url: "https://saccalcbridgehub.netlify.app/",
        description: "Interactive apps and resources for students preparing for Calculus 1 — algebra, trig, and bridge concepts.",
        tags: ["algebra", "calculus", "bridge", "trig", "precalc"]
      }
    ]
  }
];
